...
from octree_nerf.Method.camera import create_dodecahedron_cameras
from octree_nerf.Method.pose.nerf import nerf_matrix_to_ngp, visualize_poses
...
class NeRFDataset:
    def __init__(self, opt, device, type="train", n_test=10):
        ...
        if self.scale == -1:
            print(
                "[WARN] --data_format nerf cannot auto-choose --scale, use 1 as default."
            )
            self.scale = 1
        ...
        if self.mode == "colmap":
            ...
            # load train and val split
                ...
                with open(
                    os.path.join(self.root_path, "transforms_train.json"), "r"
                ) as f:
                    transform = json.load(f)
                with open(
                    os.path.join(self.root_path, "transforms_val.json"), "r"
                ) as f:
